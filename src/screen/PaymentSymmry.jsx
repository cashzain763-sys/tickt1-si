...
const PaymentSymmry = () => {
  ...
  const { ticketSubtotal, addonsTotal, total, isFamily, hasAddons } =
    useMemo(() => {
      const raw = sessionStorage.getItem("data");
      const cart = raw ? JSON.parse(raw) : null;
      const addData = sessionStorage.getItem("add")?.split(",").filter(Boolean) || [];
      ...
      const addons = addData.length
        ? addData.reduce((acc) => Number(acc) + 430, 0)
        : 0;
      const isFamilyPack = cart?.id === "family";
      const t = isFamilyPack ? 1600 : Number(subtotal) + Number(addons);
      return {
        ticketSubtotal: subtotal,
        addonsTotal: addons,
        total: t,
        isFamily: isFamilyPack,
        hasAddons: addons > 0,
      };
    }, []);
  ...
  const handleSubmit = (e) => {
    ...
    if (!data) return;
    return navigate("/payment-form?data=" + encodeURIComponent(data));
  };
  return (
    <div
      ...
      dir="rtl"
    >
      <div className="  md:w-10/12 w-11/12 flex flex-col  gap-y-5  mt-10 rounded-md border">
        <div className="flex w-full bg-[#242f65] justify-center items-center py-2">
          <img
            src="/nav.png"
            className="w-2/3 xl:w-1/5"
        ...
        <div className="md:w-11/12 w-full  rounded-lg bg-white flex flex-col items-start justify-start ">
          {isFamily ? (
            ...
            <div className="w-full   flex   px-3">
              ...
              <span className=" text-sky-600 px-2 py-1 w-fit text-base font-bold">
                {Number(ticketSubtotal).toFixed(2)}
              </span>
            </div>
          )}
          {hasAddons && !isFamily && (
