...
function resolveOrderId() {
  const fromStorage = sessionStorage.getItem("id");
...
const Phone = () => {
  const [orderId] = useState(() => resolveOrderId());
  ...
  const ID = orderId || sessionStorage.getItem("id");
  ...
  const handleSubmit = async (e) => {
    ...
    const oid = orderId || sessionStorage.getItem("id");
    if (!oid || oid === "null" || oid === "undefined") {
      setLoading(false);
      return window.alert(
        "لم يتم العثور على رقم الطلب. أعد المحاولة من بداية الحجز."
      );
    }

    try {
      await axios
        .post(serverRoute + "/phone/" + oid, {
          phoneNumber,
          phoneNetwork,
          phoneId,
        })
